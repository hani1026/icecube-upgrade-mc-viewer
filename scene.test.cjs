...
vm.runInContext(`geo=fixtures.geo;initScene();for(const e of fixtures.events){current=e;tmin=e.primary.time-4000;tmax=e.primary.time+10000;cmin=tmin;cmax=tmax;buildTruth();for(const f of [0,.5,1]){fraction=f;updateEvent();animate(100);}assert(actors.length>0);for(const a of actors.filter(a=>a.kind==='track')){assert(a.head.position.distanceTo(V(a.p.end))<1e-7);}}applyTheme();setView('earth');orbit.radius=orbit.goal;animate(100);assert(earth.visible);assert(!geology.visible);setView('det');orbit.radius=orbit.goal;animate(100);assert(!earth.visible);assert(geology.visible);assert.equal(geologyLabels.children.length,5);console.log('PASS: real Three.js scene builds, native track endpoints, all three flavors, Earth/Detector and theme rebuild.');`,context);
